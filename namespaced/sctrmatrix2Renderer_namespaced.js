...
ScatterMatrix.prototype.render = function () {
  ...
  var container = d3.select(this.__dom_id)
                    .attr('class', 'avf-scatter-matrix-container');
  var svg = container.append('div')
                     .attr('class', 'avf-scatter-matrix-svg')
                     .html('<em>Loading data...</em>');

  var control = container.append('div')
                         .attr('class', 'avf-scatter-matrix-control');
  ...
  this.onData(function() {
    ...
    var size_control = control.append('div').attr('class', 'avf-scatter-matrix-size-control');
    var brush_tooltip = control.append('div').attr('class', 'avf-scatter-matrix-brush-tooltip');
    var color_control = control.append('div').attr('class', 'avf-scatter-matrix-color-control');
    var filter_control = control.append('div').attr('class', 'avf-scatter-matrix-filter-control');
    var variable_control = control.append('div').attr('class', 'avf-scatter-matrix-variable-control');
    var drill_control = control.append('div').attr('class', 'avf-scatter-matrix-drill-control');
...
ScatterMatrix.prototype.__draw =
  ...
  this.onData(function() {
    ...
    function color_class(d) {
      ...
      return colors.length > 0 ? 'avf-scatter-matrix-color-'+colors.indexOf(c) : 
        'avf-scatter-matrix-color-2';
    }
    ...
    var legend = svg.selectAll("g.avf-scatter-matrix-legend")
        .data(colors)
      .enter().append("svg:g")
        .attr("class", "avf-scatter-matrix-legend")
    ...
    // Draw X-axis
    svg.selectAll("g.x.avf-scatter-matrix-axis")
        .data(x_variables)
      .enter().append("svg:g")
        .attr("class", "x avf-scatter-matrix-axis")
        .attr("transform", function(d, i) { return "translate(" + i * size + ",0)"; })
        .each(function(d) { d3.select(this).call(x_axis.scale(x[d]).orient("bottom")); });

    // Draw Y-axis
    svg.selectAll("g.y.avf-scatter-matrix-axis")
        .data(y_variables)
      .enter().append("svg:g")
        .attr("class", "y avf-scatter-matrix-axis")
        .attr("transform", function(d, i) { return "translate(0," + i * size + ")"; })
        .each(function(d) { d3.select(this).call(y_axis.scale(y[d]).orient("right")); });
    // Draw scatter plot

    var cell = svg.selectAll("g.avf-scatter-matrix-cell")
        .data(cross(x_variables, y_variables))
      .enter().append("svg:g")
        // .on("mouseover", function () { console.log(this); })
        .attr("class", "avf-scatter-matrix-cell")
    ...
    function plot(p) {
      ...
      cell.append("svg:rect")
          .attr("class", "avf-scatter-matrix__frame")
          .attr("x", padding / 2)
          .attr("y", padding / 2)
      ...
      //Display tooltip with appropriate color    
      points.on("mouseover", function (d) { 
        var circlethis = this;
        var color_class = document.querySelector("." + circlethis.getAttribute("class"));
        var color = getComputedStyle(color_class) ? getComputedStyle(color_class).fill : null;
        if (self.getContext() && color) {
      ...
      points.on("mousedown", function () {
        ...
        cell.node().dispatchEvent(new_click_event);
      });
    ...
    function brush(p) {
      var e = brush.extent();
      svg.selectAll(".avf-scatter-matrix-cell circle").attr("class", function(d) {
    ...
    function brushend() {
      if (brush.empty()) svg.selectAll(".avf-scatter-matrix-cell circle").attr("class", function(d) {
...
ScatterMatrix.prototype.highlightAxes = function () {
  d3.selectAll("g line").each(function () {
    if (this.nextSibling && this.nextSibling.innerHTML == "0") {
